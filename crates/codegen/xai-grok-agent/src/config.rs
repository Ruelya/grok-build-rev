...
///
/// The enum covers all built-in agents for centralized name management and
/// `by_name()` dispatch. `subagent_variants()` returns every cataloged built-in
/// so they are spawnable/selectable; per-agent hide still applies via
/// `[subagents.toggle]` / disallow lists. `default_enabled()` keeps the stock
/// five on and extended fork variants off until toggled.
#[derive(
