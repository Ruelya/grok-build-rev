...
fn manual_install_cmd(channel: &str) -> String {
    if xai_grok_version::is_fork_build() {
        return format!("npm install -g {}", npm_package_name());
    }
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(version_for_update_check);
    if needs_update(
        &effective_current,
        &target,
        &update_config.channel,
...
pub async fn run_install_script(
    ...
    if xai_grok_version::is_fork_build() && installer != "npm" && installer != "gh-release" {
        anyhow::bail!(
            "Fork builds cannot use the official x.ai auto-updater.              Use: npm i -g {}",
            npm_package_name()
        );
    }
...
fn fork_release_asset_name() -> Result<String> {
    let key = if cfg!(all(target_os = "windows", target_arch = "x86_64")) {
        ...
        anyhow::bail!(
            "This platform is not published for grok-build-rev.              Supported: win32-x64, linux-x64, linux-arm64, darwin-arm64.              Override: npm i -g @ruelya/grok-build or GROK_FORK_BIN=..."
        );
    };
...
async fn install_internal(target: Option<&str>, update_config: &UpdateConfig) -> Result<String> {
    if xai_grok_version::is_fork_build() {
        anyhow::bail!(
            "Official CDN install is disabled in grok-build-rev.              Install/update with: npm i -g {}",
            npm_package_name()
        );
    }
