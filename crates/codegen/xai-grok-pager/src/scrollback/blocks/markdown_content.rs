...
struct RenderState {
    ...
    /// Cached word-wrap result keyed on `(width, generation, theme_gen)`.
    cache_width: usize,
    cache_generation: u64,
    /// [`theme_cache::generation`] when styles/syntax were last applied.
    /// Uses generation (not ThemeKind) so custom theme switches re-render.
    cache_theme_gen: u64,
...
impl MarkdownContent {
    ...
    fn new_inner(
        ...
        Self {
            state: RefCell::new(RenderState {
                ...
                cache_width: 0,
                cache_generation: 0,
                cache_theme_gen: theme_cache::generation(),
                cache_lines: Vec::new(),
    ...
    fn ensure_wrapped(&self, width: usize) {
        ...
        // If the theme changed (builtin or custom), update md style + drop
        // open-code HL cache so the re-render picks up new md_* and syntax
        // palette colors. ThemeKind alone is insufficient: custom themes share
        // a base kind and would never invalidate.
        if state.cache_theme_gen != current_theme_gen {
            state.renderer.set_style(md_style::style());
            state.cache_theme_gen = current_theme_gen;
