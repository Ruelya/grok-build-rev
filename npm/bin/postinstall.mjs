...
if (process.env.GROK_FORK_SKIP_POSTINSTALL === "1") {
  console.log("[grok-build-rev] skip postinstall (GROK_FORK_SKIP_POSTINSTALL=1)");
  process.exit(0);
}
...
if (process.env.CI === "true" && process.env.GROK_FORK_FORCE_POSTINSTALL !== "1") {
  console.log("[grok-build-rev] CI detected — skip postinstall (set GROK_FORK_FORCE_POSTINSTALL=1 to run)");
  process.exit(0);
}
