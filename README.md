...
```bash
npm install -g .                         # or: npm install -g grok-build-rev-1.0.0.tgz
grok --version                           # expect …-rev (…) [stable]
npx grok-build-rev restore               # restore previous binary from backup
GROK_FORK_SKIP_POSTINSTALL=1 npm install -g .   # CI / pack without installing binary
```
...
```bash
npm install -g .                         # 或: npm install -g grok-build-rev-1.0.0.tgz
grok --version                           # 期望 …-rev (…) [stable]
npx grok-build-rev restore               # 从备份恢复上一份二进制
GROK_FORK_SKIP_POSTINSTALL=1 npm install -g .   # CI / 打包时不安装二进制
```
